        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-10">
          {skills.map((skill, index) => (
            <motion.div
              ...
              key={index}
            >
              <Card className="p-4 sm:p-8 text-center backdrop-blur-sm bg-white/5 hover:bg-white/10 transform transition-all duration-500 hover:scale-105 border border-white/10 rounded-2xl shadow-xl hover:shadow-cyan-500/20 flex flex-col items-center space-y-3 sm:space-y-6">
                <img
                  src={skill.icon}
                  alt={`${skill.name} icon`}
